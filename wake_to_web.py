        ...
        try:
            if 'sp' in globals():
                ...
                if current and current.get('item'):
                    ...
                    duration_ms = current['item'].get('duration_ms', 1)
                    track_name = current['item']['name']
                    artist_name = current['item']['artists'][0]['name']
                    
                    images = current['item']['album']['images']
                    album_art_url = images[0]['url'] if images else "https://misc.scdn.co/liked-songs/liked-songs-300.png"
                    display_name = track_name if is_playing else f"{track_name} (Paused)"
...
# --- SETUP GOOGLE CALENDAR (For Voice Commands Only) ---
...
# --- NEW: LIVE TO-DO SYNC ENGINE ---
def todo_sync_thread():
    """Silently watches the todo.json file and pushes updates to the mirror if it changes."""
    last_todos = None
    while True:
        try:
            current_todos = get_todos()
            # Only push to UI if the file actually changed to save bandwidth
            if current_todos != last_todos:
                send_to_ui({"todos": current_todos})
                last_todos = current_todos
        except Exception:
            pass
        # Check the database file every 2 seconds
        time.sleep(2)
