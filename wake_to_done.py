...
def unified_vision_thread():
    global recognized_user, last_seen_time
...
if not creds or not creds.valid:
    if creds and creds.expired and creds.refresh_token: creds.refresh(Request())
        ...
        if not os.path.exists('credentials.json'): sys.exit("\n❌ CRITICAL ERROR: 'credentials.json' is missing!")
...
def get_todos():
    if not db: return []
    try:
        doc = db.collection('todos').document('list').get()
...
def save_todos(todos):
    if not db: return
    try:
        db.collection('todos').document('list').set({'tasks': todos})
