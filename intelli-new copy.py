...
def ask_gemini(text_query):
    ...
    study_keywords = ["help me study this", "explain this problem", "tutor me on this"]
    if any(k in query_lower for k in vision_keywords + study_keywords):
        if latest_frame is not None:
            try:
                rgb_image = cv2.cvtColor(latest_frame, cv2.COLOR_BGR2RGB)
                pil_image = Image.fromarray(rgb_image)
                if any(k in query_lower for k in study_keywords):
                    prompt = f"{sys_aw}You are a Socratic tutor. Look at the homework. RULE 1: If it's too blurry, say 'I can't quite read that, hold it closer.' RULE 2: If the student is asking for help starting, DO NOT give the final answer. Ask exactly ONE guiding question. RULE 3: If the student just gave you the correct answer or figured out the final step, CONGRATULATE them, summarize the final answer clearly, and DO NOT ask any more questions. Max 3 sentences."
                else:
                ...
                # Send the image INTO the chat memory so it doesn't forget the math problem!
                res = mirror_chat.send_message([pil_image, prompt])
    ...
    try:
        res = mirror_chat.send_message(f"{sys_aw}Keep responses brief and conversational (1-2 sentences).\nUser: {text_query}")
