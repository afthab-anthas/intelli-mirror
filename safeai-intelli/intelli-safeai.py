...
last_seen_time = 0
FACE_TIMEOUT = 30  # auto logout after 1 min
...
def speak(text):
    ...
    try:
        # clean text so it doesn't read out markdown/LaTeX math symbols
...
def enroll_user(name):
    ...
    # Pi 5 Native Camera Setup
    w, h = 640, 480
    cmd = ["rpicam-vid", "-t", "0", "--width", str(w), "--height", str(h), "--framerate", "30", "--codec", "yuv420", "--inline", "--flush", "-o", "-"]
...
def unified_vision_thread():
    ...
    print("Starting Pi 5 hardware camera pipeline...")
    w, h = 640, 480
    cmd = ["rpicam-vid", "-t", "0", "--width", str(w), "--height", str(h), "--framerate", "30", "--codec", "yuv420", "--inline", "--flush", "-o", "-"]
    ...
    try:
        camera_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
        frame_size = int(w * h * 1.5)
        print("Hardware pipeline active.")
        time.sleep(1)
    except Exception as e:
        print(f"Failed to start hardware camera: {e}")
        return
    ...
    while True:
        ...
        if enrollment_active:
            if camera_process.poll() is None: # Process is still running
                print("Vision thread releasing camera for enrollment scanner...")
                camera_process.kill()
            time.sleep(0.5)
            continue
        elif camera_process.poll() is not None: # Process is dead
            print("Vision thread re-acquiring camera...")
            camera_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
            time.sleep(0.5)
            continue
        ...
        try:
            raw_data = camera_process.stdout.read(frame_size)
            if len(raw_data) != frame_size:
                print("Camera frame dropped. Restarting pipeline...")
                camera_process.kill()
                camera_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
                time.sleep(0.5)
                continue
            ...
            yuv_array = np.frombuffer(raw_data, dtype=np.uint8).reshape((int(h * 1.5), w))
            image = cv2.cvtColor(yuv_array, cv2.COLOR_YUV2BGR_I420)
        except Exception as e:
        ...
        # logout if no face seen for a while AND conversation is dormant
        if recognized_user is not None and (time.time() - last_seen_time > FACE_TIMEOUT) and (time.time() - last_interaction_time > 60):
            global mirror_chat
            recognized_user = None
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token: 
            try:
                ...
                print(f"Error refreshing calendar credentials for {recognized_user}: {e}")
...
def ask_gemini(text_query):
    ...
    # 4.5 Focus / Pomodoro Mode
    pomodoro_keywords = ["start a study session", "focus mode", "start pomodoro"]
    if any(k in query_lower for k in pomodoro_keywords):
        try:
            sp.start_playback(context_uri="spotify:playlist:0vvXsWCC9xrXsKd4FyS8kM")
        except Exception as e:
            pass
        send_to_ui({"type": "start_timer", "minutes": 25})
        return speech_queue.put("Study session started. 25 minutes on the clock. Focus up!")

    # 4.6 End Focus Mode
    stop_pomodoro_keywords = ["stop study session", "stop focus mode", "stop the timer", "end study session"]
    if any(k in query_lower for k in stop_pomodoro_keywords):
        try:
            sp.pause_playback()
        except Exception:
            pass
        send_to_ui({"type": "stop_timer"}) # Tells the frontend to hide the clock
        return speech_queue.put("Study session ended. Great work!")

    # 4 & 5. Always-On Vision & Conversational Chat
    if latest_frame is not None:
        try:
            ...
            # The Omniscient Master Prompt
            master_prompt = f"{sys_aw}You can see me through the mirror's camera. " \
                            f"RULE 1: If I am holding up math or homework, act as a strict Socratic tutor (never give the final answer, ask exactly ONE guiding question. If I get it right, congratulate me and do not ask further questions. If blurry, say you can't read it). EXCEPTION: If and only if the user insists or demands a direct answer (e.g., 'just tell me', 'what is the answer', 'give me the answer'), skip every step and just blurt out the final answer directly. " \
                            f"RULE 2: If I ask what I am holding, identify the object. " \
                            f"RULE 3: If my question has nothing to do with the camera or an object, ignore the image entirely and just chat with me normally. " \
                            f"Keep all responses brief and conversational (1-3 sentences).\nUser: {text_query}"
            
            # Send both the image AND the text into the chat memory
            res = mirror_chat.send_message([pil_image, master_prompt])
            speech_queue.put(res.text.strip())
        except Exception as e: 
            print(f"Chat error: {e}")
            speech_queue.put("I am having trouble connecting to my brain right now.")
    else:
        # Fallback if the webcam crashes
        try:
...
def start_listening(is_recalibrating=False):
    ...
    for i, mic_name in enumerate(sr.Microphone.list_microphone_names()):
        if "Jabra" in mic_name or "USB" in mic_name:
            target_index = i
            if not is_recalibrating:
                print(f"Auto-detected USB Mic at index {target_index}")
            break
            
    if target_index is None:
        sys.exit("CRITICAL: Jabra USB mic not found! Is it plugged in?")
    # -----------------------------
    
    try: 
        microphone = sr.Microphone(device_index=target_index, sample_rate=16000, chunk_size=2048)
    except OSError as e: 
        sys.exit(f"could not access microphone: {e}")
        
    with microphone as source:
        print("Calibrating USB microphone to room noise...")
        recognizer.adjust_for_ambient_noise(source, duration=2)
        if not is_recalibrating: 
            print(f"microphone ready. say '{WAKE_WORD}'")
            
    # Remove the strict phrase_time_limit so it doesn't cut you off early on the Pi
        ...
        while True:
            ...
            if not listening_for_command and not is_speaking and (time.time() - last_interaction_time) > 60:
                ...
                if has_history:
                    ...
                    # We deliberately DO NOT reset last_interaction_time here 
                    # so the vision thread knows the conversation is truly dormant.
