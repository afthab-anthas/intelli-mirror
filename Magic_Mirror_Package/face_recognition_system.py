"""
Magic Mirror OS - "Hybrid AI" Enrollment Hub
===========================================
Uses YuNet (CNN) to detect high-quality faces for the 8-Step ML Notebook.
"""
...
FACE_TIMEOUT = 10  # Seconds before auto-logout
CAPTURED_SIZE = (100, 100) # Size for the notebook's Step 1
...
def get_ai_tools(w=640, h=480):
    if not os.path.exists(YUNET_PATH): return None, None
...
def enroll_mode(name):
    ...
    if detector is None:
        print("[!] Error: yunet.onnx missing. Re-run download command.")
        return

    print(f"\n[>>] Enrolling '{name}'...")
    print("Look at the camera. Gathering 150 facial snapshots...")
    ...
    while collected < 150:
        ...
        if faces is not None:
            for face in faces:
                # Use SFace for perfect alignment (this makes the 8-steps way more accurate)
                if recognizer:
                    face_aligned = recognizer.alignCrop(frame, face)
                    # Convert to grayscale for the notebook's classic ML pipeline
                    face_gray = cv2.cvtColor(face_aligned, cv2.COLOR_BGR2GRAY)
                    face_resized = cv2.resize(face_gray, CAPTURED_SIZE)
                else:
                    # Fallback if SFace missing
    ...
    if collected >= 150:
        ...
        print(f"[M] Enrolled '{name}' with 150 samples.")
        print("[!] SYSTEM HALT: You must now open 'train_face_model.ipynb' to build the AI model!")
    else:
...
if __name__ == "__main__":
    ...
    if args.enroll:
        name = input("\nEnter name: ").strip()
        if name: enroll_mode(name)
    else:
        print("Run 'python magic_mirror_engine.py' or '--enroll'.")
